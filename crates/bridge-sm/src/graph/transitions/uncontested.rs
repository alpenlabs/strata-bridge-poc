...
impl GraphSM {
    ...
    pub(crate) fn process_adaptors_verification(
        ...
        match self.state() {
            ...
            // since this event is generated by an external service, we cannot make assumptions
            // about when this event will be generated. So we should not treat receiving this event
            // later in the state machine as an invariant violation.
            _ => Err(GSMError::rejected(
                self.state().clone(),
                adaptors.into(),
                "Adaptors verified event is inapplicable in the current state",
            )),
        }
    }
