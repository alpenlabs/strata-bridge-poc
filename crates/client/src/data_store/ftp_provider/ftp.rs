...
impl Ftp {
    pub fn new() -> Option<Self> {
        dotenvy::dotenv().ok();
        let host = dotenvy::var("BRIDGE_FTP_HOST");
        let port = dotenvy::var("BRIDGE_FTP_PORT");
        let username = dotenvy::var("BRIDGE_FTP_USERNAME");
        let password = dotenvy::var("BRIDGE_FTP_PASSWORD");
        let base_path = dotenvy::var("BRIDGE_FTP_BASE_PATH");
