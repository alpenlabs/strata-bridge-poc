...
use openssh_sftp_client::{file::TokioCompatFile, Sftp as _Sftp};
...
impl Sftp {
    pub fn new() -> Option<Self> {
        dotenvy::dotenv().ok();
        let host = dotenvy::var("BRIDGE_SFTP_HOST");
        let port = dotenvy::var("BRIDGE_SFTP_PORT");
        let username = dotenvy::var("BRIDGE_SFTP_USERNAME");
        let keyfile_path = dotenvy::var("BRIDGE_SFTP_KEYFILE_PATH");
        let base_path = dotenvy::var("BRIDGE_SFTP_BASE_PATH");
        ...
        let credentials = SftpCredentials {
            ...
            _keyfile_path: keyfile_path.unwrap(),
