...
use strata_bridge_contexts::{
    base::generate_n_of_n_public_key, depositor::DepositorContext, operator::OperatorContext,
...
pub type SecNonceTable = HashMap<usize, SecNonce>;
pub type TxidToSecNonce = HashMap<Txid, SecNonceTable>;
pub type GraphToTxid = HashMap<String, TxidToSecNonce>;
pub type PublicKeyToGraph = HashMap<PublicKey, GraphToTxid>;
...
impl BitVMClient {
    ...
    */
    ...
    pub async fn broadcast_claim(&mut self, peg_out_graph_id: &str) {
        let peg_out_graph = self
            .data
            .peg_out_graphs
            .iter_mut()
        ...
        peg_out_graph.unwrap().claim(&self.esplora).await;
    }
    ...
    pub async fn broadcast_payout_optimistic(&mut self, peg_out_graph_id: &str) {
        let peg_out_graph = self
            .data
            .peg_out_graphs
            .iter_mut()
        ...
        peg_out_graph
            .unwrap()
            .payout_optimistic(&self.esplora)
            .await;
    }

    pub async fn broadcast_payout(&mut self, peg_out_graph_id: &str) {
        let peg_out_graph = self
            .data
            .peg_out_graphs
            .iter_mut()
        ...
        peg_out_graph.unwrap().payout(&self.esplora).await;
    }
    ...
    pub fn push_peg_in_nonces(&mut self, peg_in_graph_id: &str) {
        ...
        // TODO: Save secret nonces for all txs in the graph to the local file system. Later, when
        // pre-signing the tx, we'll need to retrieve these nonces for this graph ID.
