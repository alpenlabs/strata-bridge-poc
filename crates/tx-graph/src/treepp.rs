...
pub fn execute_script(script: Script) -> ExecuteInfo {
...
pub fn run(script: Script) {
    ...
    assert!(
        stack.is_valid_final_state_without_inputs(),
        "Stack analysis does not end in valid state: {:?}",
        stack
    );
...
pub fn run_as_chunks(script: Script, chunk_size: usize, stack_limit: usize) {
...
pub fn execute_script_without_stack_limit(script: Script) -> ExecuteInfo {
...
pub fn execute_script_as_chunks(
    ...
    assert!(!scripts.is_empty(), "No chunks to execute");
    ...
    let mut final_exec = None; // Only used to not initialize an obsolete Exec
    ...
    for i in 0..num_chunks {
        ...
        // TODO: Take the stack snapshot at the end of the chunk logic (before the stack is hashed
        // and then dropped) BUT AFTER THE ALSTACK IS MOVED TO STACK
...
pub fn execute_script_as_chunks_vs_normal(
    ...
    let mut total_scripts = vec![];
    for script in &scripts {
        total_scripts.extend(script.clone().into_bytes());
    }
    println!("chunk sizes: {:?}", chunk_sizes);

    for (i, total_script) in total_scripts
        .iter()
        .enumerate()
        .take(min(compiled_script.len(), total_scripts.len()))
    {
        assert_eq!(
    ...
    //    "Total chunk script is not same as compiled script {:?}, {:?}", compiled_script.len(),
    // total_script.len()
    //);
